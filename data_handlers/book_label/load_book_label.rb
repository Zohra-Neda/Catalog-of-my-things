...
module LoadBookAndLabelData
  ...
  def load_labels
    label_hash = []
    return label_hash unless File.exist?('store_data/label.json')

    label_hash = load_data_from_file('store_data/label.json')
  ...
  def load_books
    book_hash = []
    return book_hash unless File.exist?('store_data/book.json')

    book_hash = load_data_from_file('store_data/book.json')
